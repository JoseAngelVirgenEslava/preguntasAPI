    ...
    <div className="max-w-2xl mx-auto p-4">
      ...
      <div className="p-6 bg-white rounded-lg shadow-xl">
        ...
        {preguntaActual.tipo_pregunta === "opcion_multiple" ? (
          ...
          <p className="text-red-500">Error: Tipo de pregunta no reconocido o estado de datos inesperado.</p>        )}
      </div>
          ...
          <button
            onClick={async () => {
              try {
                ...
                if (!resp.ok) {
                    const errData = await resp.json().catch(() => ({error: "Error al evaluar cuestionario"}));
                    throw new Error(errData.error || "Error al evaluar");
                }
                const data = await resp.json();
                const numPreguntasContestadas = preguntas.length;
